	 ...
	 public static void load_json_auto(){
		 ...
		 try {
       	  XStream xstream = new XStream(new JettisonMappedXmlDriver());
			  ...
			  xstream.alias("admin", admin.class);
			  if(SingletonF.configApp.getDummiesmode())
	        	  pathin="/src/application/modules/users/model/files/dummies/admin_files/admins.json";
	          else
	        	  pathin="/src/application/modules/users/model/files/admin_files/admins.json";
			  PATH = new java.io.File(".").getCanonicalPath()
	                    + pathin;
